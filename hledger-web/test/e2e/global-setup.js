// Start hledger-web for the e2e run, on a scratch copy of fixture.journal
...
module.exports = async () => {
  const journal = path.join(os.tmpdir(), `hledger-web-e2e-${process.pid}.journal`);
  fs.copyFileSync(path.join(__dirname, 'fixture.journal'), journal);
  process.env.E2E_JOURNAL = journal;
  ...
  child.unref();
  fs.writeFileSync(path.join(os.tmpdir(), 'hledger-web-e2e.pid'), String(child.pid));
